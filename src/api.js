// Mapa de países del ICS a códigos alpha-2 para extraer las banderas
const flagMap = {
...
const parseICS = (icsString) => {
  ...
  for (let line of lines) {
    ...
    if (line === 'BEGIN:VEVENT') {
      ...
      // Solo agregamos el evento si realmente tiene fecha de inicio
  ...
  const cleanName = (name) => {
    ...
    // Eliminar emojis de manera ultra segura (sin romper el compilador)
    return name.replace(/[^\w\s\-\u00C0-\u00FF/]/g, '').replace(/_/g, '').trim();
  };

  return events.map((ev, index) => {
    let summary = ev.summary || '';
    let cleanSummary = cleanName(summary);
    
    const parts = summary.split(' - ');
    let homeName = parts[0] ? cleanName(parts[0]) : 'Por Definir';
    let awayName = parts[1] ? cleanName(parts[1]) : 'Por Definir';

    let homeScore = null;
    let awayScore = null;

    // Detectar si el texto tiene resultados (ej: "Mexico 2 - 1 South Africa")
    const homeScoreMatch = homeName.match(/\s(\d+)$/);
    if (homeScoreMatch) {
      homeScore = parseInt(homeScoreMatch[1], 10);
      homeName = homeName.replace(/\s\d+$/, '').trim();
    }
    const awayScoreMatch = awayName.match(/^(\d+)\s/);
    if (awayScoreMatch) {
      awayScore = parseInt(awayScoreMatch[1], 10);
      awayName = awayName.replace(/^\d+\s/, '').trim();
    }

    // Deducir fase del torneo según el título del partido
    let groupName = 'Fase de Grupos';
    if (cleanSummary.includes('Winner SF')) groupName = 'Final';
    else if (cleanSummary.includes('Loser SF')) groupName = 'Tercer Puesto';
    else if (cleanSummary.includes('Winner QF')) groupName = 'Semifinales';
    else if (cleanSummary.includes('Winner EF')) groupName = 'Cuartos de Final';
    else if (cleanSummary.includes('/')) groupName = 'Octavos de Final';
    else if (cleanSummary.match(/^[1-2][A-L]/)) groupName = 'Dieciseisavos de Final';

    // Convertir a Objeto Date parseando estrictamente a enteros
...
export const fetchMatches = async () => {
  ...
  return activeFixtures.length > 0 ? activeFixtures : allMatches;
};
...
export const fetchResults = async () => {
  ...
  if (pastMatches.length === 0) {
    return [{ date: 'Aún no hay resultados', matches: [] }];
  }

  const grouped = pastMatches.reduce((acc, match) => {
    const dateStr = new Date(match.rawDate).toLocaleDateString([], { weekday: 'long', day: 'numeric', month: 'long' });
...
SEQUENCE:0
SUMMARY:🇲🇽 Mexico 3 - 1 🇿🇦 South Africa
TRANSP:TRANSPARENT
